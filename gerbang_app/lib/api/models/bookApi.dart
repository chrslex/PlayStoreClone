...
static Future<Book> getBooksById(String id) async {
  ...
  static Future<List<String>?> getBookSubcategories() async {
    final response = await http.get(Uri.parse(
        route + "api/v1/subcategories?subtype=book"));
    if (response.statusCode == 200) {
      // If the server did return a 200 OK response,
      // then parse the JSON.
      var data = jsonDecode(response.body);
  ...
  static Future<List<Book>> getBooksBySubcategory(String subcategory, int page) async {
    final response = await http.get(Uri.parse(
        route + "api/v1/product/books?page=$page&page_size=5&subcategory=$subcategory"));
    if (response.statusCode == 200) {
      // If the server did return a 200 OK response,
      // then parse the JSON.
      var data = jsonDecode(response.body);
